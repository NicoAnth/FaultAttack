...
std::vector<std::string> faultAttackf(){
    ...
    std::string plainText = "64706BDA3B79FDD0";
 ...
 */
    ...
    for(int i=0; i<8;i++){
        ...
        std::vector<std::pair<std::string,int>> kO = keyOccurrence(possibleKeys[i]);
...
int main(int argc , char** argv) {
    ...
    //Decompress the key in order to find pc2^-1(K16)
    std::string pc2Reverse = decompressKeyPC2(K16s);

    //Compute pc1^-1(pc2^-1(K16))
    std::string pc1Reverse = decompressKeyPC1(pc2Reverse);
    
    //Get unknown bits position
    std::vector<int> unknownBits = findUnknownBits(pc1Reverse);

    //Generate every candidate key
    std::vector<std::string> candidateKeys = generateEveryPossibleK(pc1Reverse);

    //Compute DES on every candidate key and print the correct key
    for(int i=0; i<256;i++){
        generateKeys(candidateKeys[i]);
        std::string cypher = DES();
        if(cypher == hexStrToBinStr("144FED14635AFD89")){
            std::cout<<"clé:"<<candidateKeys[i]<<std::endl;
            break;
        }
    }
